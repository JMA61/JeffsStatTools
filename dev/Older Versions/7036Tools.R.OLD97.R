...
.jst_missing_comma_error <- function(data_expr_str, fn_name, original_error) {
  ...
  if (!is.null(default_df)) {
    stop(paste0(
      ...
      "Otherwise, check spelling and capitalisation."
    ), call. = FALSE)
  } else {
...
#' jdesc(, mpg, hp)       # Uses mtcars automatically; data-first syntax
#' jt(mpg ~ am)           # Uses mtcars automatically; formula syntax (data= not needed)
    ...
    if (!is.null(orig_val_labels) && length(orig_val_labels) > 0) {
      ...
      if (is_collapsing) {
        ...
        if (length(new_val_labels) > 0) {
          ...
          message("Value labels have been automatically transferred to the recoded ",
                  "variable — use jfreq() to inspect & jrelabel() to update if needed.")
        }
